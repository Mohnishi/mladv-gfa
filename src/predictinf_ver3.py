...
groupmax=50 # maximum group number
...
for changegroup in range(0,(groupmax-groupmin)//groupcounter+1,1):
        ...
        for trial in range(0,trialmax,1):
                ...
                while (W.max() > 100):#or(X.max() > 100):
                        X, W, Z, alpha, Tau = generation(N, K, D, R)
                # Run GFA
                g = gfa.GFA(rank=R, factors=K,optimize_method="l-bfgs-b", debug=True, max_iter=100)
                g.fit(X,D)
                ...
                Xmpre=Yminus @ T @ Wminus.T @ np.linalg.inv(Sigma) @ Wm  # (N x D-Dm)x(D-Dm x D-Dm)x(D-Dm x K)x(K x K)x(K x Dm)=(N x Dm)
               
                if trial==0:
                        preerr=np.sum((X.T[:,Leave*Dm:(Leave+1)*Dm]-Xmpre)**2)  # squared error of the prediction = scaler
                        RMSE[changegroup]=preerr  # summing (devided by trialmax afterward)
                elif np.sum((X.T[:,Leave*Dm:(Leave+1)*Dm]-Xmpre)**2)<preerr:
                        preerr=np.sum((X.T[:,Leave*Dm:(Leave+1)*Dm]-Xmpre)**2)  # squared error of the prediction = scaler
                        RMSE[changegroup]=preerr  # summing (devided by trialmax afterward)
                print(preerr)
